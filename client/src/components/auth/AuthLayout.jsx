 ...
 *
 * Two-panel design on wide screens (≥ 1024px):
 *   • Left  — branded marketing panel (hidden on mobile/tablet)
 *   • Right — the actual form content via <Outlet />
 *
 * On mobile, the left panel is unmounted entirely and the right
 * panel takes the full viewport, with the brand logo surfaced at
 * the top of the form so users still see the AttendX identity.
 *
 * Responsive behaviour uses a useIsDesktop hook (matchMedia) rather
 * than CSS classes, so the result is the same regardless of whether
 * Tailwind is processing the className attributes.
 * ═════════════════════════════════════════════════════════════════
 */
...
export default function AuthLayout() {
  ...
  return (
    <div style={{
      ...
      {/* ═══ Left: decorative brand panel — desktop only ═══ */}
      {isDesktop && (
        <div
          style={{
            display:        'flex',
      ...
      <div style={{
        ...
        <div style={{
          ...
          {/* Mobile logo — only when the left panel is hidden */}
          {!isDesktop && (
            <div style={{
              display:      'flex',
              alignItems:   'center',
