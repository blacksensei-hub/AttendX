...
// ─── Shared nav + footer content ────────────────────────────
// Rendered inside both the desktop sidebar and the mobile overlay.
...
export default function Sidebar({ mobileOpen = false, onClose }) {
  ...
  const contentProps = {
    collapsed, isMobile, navItems, badges, user,
  ...
  return (
    <motion.aside
      ...
      transition={{
        x:       { duration: DURATION.slow, ease: EASE.entry },
        opacity: { duration: DURATION.slow, ease: EASE.entry },
        width:   SPRING.gentle,
      }}
      style={{
        display:         'flex',
  ...
  return (
    <>
      <div style={{
        ...
        <div style={{
          width: '28px', height: '28px', background: 'var(--brand-subtle)',
          borderRadius: 'var(--radius-atomic)', display: 'flex',
        ...
        <div style={{ minWidth: 0, flex: 1 }}>
          <p style={{ color: 'var(--text-primary)', fontSize: 'var(--text-xs)', fontWeight: 500, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', lineHeight: 1.3 }}>
            {user?.name}
          </p>
          <p style={{ color: 'var(--text-muted)', fontSize: '10px', textTransform: 'capitalize', letterSpacing: '0.04em' }}>
            {user?.role}
          </p>
        </div>
        <ThemeToggle />
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '6px' }}>
      <div title={user?.name} style={{
        width: '32px', height: '32px', background: 'var(--brand-subtle)',
        border: '1px solid var(--brand-border)', borderRadius: 'var(--radius-atomic)',
        display: 'flex', alignItems: 'center', justifyContent: 'center',
        fontWeight: 700, fontSize: 'var(--text-xs)', color: 'var(--brand-text)',
  ...
  return (
    ...
    >
      <motion.button
        ...
        style={{
          width: '32px', height: '32px', borderRadius: 'var(--radius-atomic)',
          background: hovered && danger ? 'var(--red-bg)' : 'transparent',
          border: 'none', cursor: 'pointer',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          color: hovered && danger ? 'var(--red)' : 'var(--text-muted)',
          transition: `background ${DURATION.fast}ms ${EASE.state}, color ${DURATION.fast}ms ${EASE.state}`,
        }}
      >
        <Icon size={16} />
      </motion.button>
...
function NavItem({ to, icon: Icon, label, badgeCount, collapsed }) {
  ...
  return (
    <div style={{ position: 'relative' }}>
            ...
            {badgeCount > 0 && (collapsed ? (
              <motion.span
                ...
                style={{
                  ...
                  boxShadow: '0 0 0 2px var(--sidebar-bg)', zIndex: 2,
                }}
              />
            ) : (
              <motion.span
      ...
      <AnimatePresence>
        {collapsed && hovered && <Tooltip label={label} badgeCount={badgeCount} />}
      </AnimatePresence>
    </div>
  );
}
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        transform: 'translateY(-50%)', marginLeft: '12px',
        background: 'var(--bg-card)', border: '1px solid var(--border)',
        borderRadius: 'var(--radius-atomic)', padding: '6px 12px',
        boxShadow: 'var(--shadow-md)', whiteSpace: 'nowrap', zIndex: 100,
    ...
    >
      ...
      {badgeCount > 0 && (
        <span style={{
          ...
          fontWeight: 700, padding: '1px 6px', minWidth: '16px',
